...
export default function AuthGate({ onLogin, onSkip }: AuthGateProps) {
  ...
  const handleError = (e: any) => {
    ...
    else if (message.includes('api-key-expired')) message = "Firebase API Anahtarı süresi dolmuş. Lütfen teknik destek ile iletişime geçin.";
    else if (message.includes('popup-closed-by-user')) return; // Ignore user closing popup
    setError(message);
  };
          ...
          <>
            ...
            <div className="flex gap-3 justify-center mb-6">
              <button 
                ...
                className="flex items-center justify-center w-14 h-14 bg-white hover:bg-[#f8f9fa] border border-[#eaeaea] rounded-2xl hover:scale-105 transition-all shadow-sm disabled:opacity-50"
              >
                {googleLoading ? (
                  <Loader2 className="w-6 h-6 animate-spin text-[#7a6090]" />
                ) : (
